...
namespace CloudConsole
{
    ...
    public class GenericKeywords
    {
        ...
        public static int implicitlyWaitTime = 60;
        public static string timestamp = DateTime.Now.ToString("yy-MM-dd hh-mm-ss");
        public static int elementLoadWaitTime = 30;
        public static String screenshot;
        public static String FilePathScreenshot = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "");
        public static String Report = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, timestamp+ "_Console.html");
        public static String ChromeDriver = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "chromedriver");
        ...
        public void onetime()
        {
            try
            {
                Console.WriteLine("You are in the temp directory."+Report);
                extd = new ExtentReports(Report);
        ...
        public static void testLogFail(String ErrorMessage)
        {
            ...
            String Name = timestamp + "Automation";
            Screenshot ss;
            ss = ((ITakesScreenshot)driver).GetScreenshot();
            ss.SaveAsFile(Name+".png", ScreenshotImageFormat.Png);
            test.Log(LogStatus.Error, test.AddScreenCapture(screenshot));
            Assert.Fail();
        }
        public static void openBrowser(String URL)
        {
            try
            {
                ...
                driver.Manage().Timeouts().ImplicitWait = TimeSpan.FromSeconds(implicitlyWaitTime);
                //driver.Manage().Cookies.DeleteAllCookies();
                driver.Manage().Window.Maximize();
                testLogPass("Chrome Browser launched");
        ...
        public static void verifyElementText(String objectLocator, String expectedText)
        {
            ...
            try
            {
                ...
                {
                    testLogPass("verify if the '" + webElement.Text.Trim() + "' element contains text '" + expectedText + "'");
                }
                else
                {
                    testLogFail("verify if the '" + webElement.Text.Trim() + "' element contains text '" + expectedText);
                }
            }
            catch (Exception e)
            {
        ...
        public static String getAttributeValue(String objLocator, String attributeName)
        {
            ...
            for (int i = 1; i <= elementLoadWaitTime; i++)
            {
                try
                {
                    ...
                    getAttributeValues = webElement.GetAttribute(attributeName);
                    testLogPass("Sucessfully got the attribute value '" + getAttributeValues + "'");
                    break;
                }
                catch (InvalidSelectorException e)
                {
        ...
        public static void selectFromDropdown(String objLocator, String valueToSelect)
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    testLogPass("Select '" + valueToSelect + "' option from : " + locatorDescription);
                }
                catch (InvalidSelectorException e)
                {
                    waitTime(1L);
        ...
        public static void selectFromDropdown(String objLocator, int index)
        {
            waitForElementToDisplay(objLocator, elementLoadWaitTime);
            try
            {
                ...
                try
                {
                    ...
                    testLogPass("Select '" + index + "' option from : " + locatorDescription);
                }
                catch (InvalidSelectorException e)
                {
                    waitTime(1L);
        ...
        public static Boolean isElementDisplayed(String objectLocator)
        {
            ...
            if (webElement.Displayed)
            {
                testLogPass("Verified that " + locatorDescription + " is Displayed");
                return true;
            }
            else
            {
                testLogFail(locatorDescription + " is not displayed");
                return false;
            }
        }
